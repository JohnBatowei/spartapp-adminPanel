    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          "http://localhost:3800/api/read-create-store",
          {
            headers: {
...
//    alert(selectOption)
    ...
    const handleCreateFormSubmit = async event => {
      ...
      if (!storeDesign) {
        setEe("Please select a store design display.");
        // setErrorMessage("Please select an image file.");
        setSuccessMessage("");
        setSs("");
        return;
      } else if (storeDesc.trim() === "") {
        ...
        setEe("");
        // setErrorMessage("");
        let formData = JSON.stringify({storeDesign, storeDesc})
        // console.log(formData);
  
        const connection =
          "http://localhost:3800/api/create-store" || "";
        try {
    ...
    const handleSeeButtonClick = async (storeId) => {
      ...
      try {
        ...
        const response = await axios.get(`http://localhost:3800/api/get-create-store-products/${storeId}`, {
          headers: {
    ...
    const handleDelete = async itemId => {
      try {
        ...
        const res = await fetch(
          `http://localhost:3800/api/delete-create-store/${itemId}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`
  ...
  //--------------------------------------------------------------------------------------------------------
    ...
    // console.log(getCategoryValue[0].categoryId)
      ...
      if (discountPercentage === undefined || discAmount === undefined || discounted === undefined) {
        ...
        try {
          const res = await axios.post(
            "http://localhost:3800/api/create-create-store-product",
            formData,
            {
              headers: {
                Authorization: `Bearer ${token}`
  ...
  // console.log(productEditStates[productID])
    ...
    try {
      ...
      const res = await axios.patch(
        "http://localhost:3800/api/update-create-store-product",
        updateFormData,
        {
          headers: {
            Authorization: `Bearer ${token}` 
  ...
  const handleDeleteProduct = async (productID,storeId) => {
    try {
      ...
      const res = await axios.delete(
        `http://localhost:3800/api/delete-create-store-product/${storeId}/${productID}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        ...
        <form className="createForm" onSubmit={handleCreateFormSubmit}>
          ...
          <div className="upload">
            ...
            <div className="control-input">
                  <select onChange={(e) => handleSelectDesign(e)}>
                    <option value="">Select store design</option>
                    <option value="regular">Regular</option>
                    <option value="random">Random</option>
                    <option value="spartAppBestPrices">spartApp best prices</option>
                    <option value="flashSales">Flash sales</option>
                    <option value="topDealsOneWay">top deals (no see all link)</option>
                  </select> <button>create now</button>
              <br />
              <div>
                <label>Describe store name : </label>
                <input
                  name="displayScreen"
                  id="displayScreen"
                  value={storeDesc}
                ...
                {paginateData().map((item, index) =>
                  <React.Fragment key={item._id}>
                    <tr>
                      ...
                      <td>{item.storeDesign}</td> {/* Adjust this line if needed */}
                      {/* <td>
                          ...
                          <form onSubmit={(event) => handlUploadProduct(event, item._id, index)}>
                            ...
                            <div className="pAmount">
                              ...
                              <div className="radio qty">
                                ...
                                onChange={(e)=>handleProductTypeChange(index,e,'new')}/>new  
                                <input type="radio" value={productStates[index]?.productType } name="type"  
